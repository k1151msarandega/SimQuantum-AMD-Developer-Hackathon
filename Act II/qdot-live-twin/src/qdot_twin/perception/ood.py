...
Design decision on window adaptation (flagged as open during planning):
the window ALWAYS accepts new frames, regardless of whether they were
flagged anomalous. This means after a real jump, the detector correctly
keeps flagging for roughly one window's worth of frames (while the window
is still dominated by pre-jump statistics), then naturally quiets down as
the window fills with post-jump frames and the new state becomes the
baseline. Simpler and more honest than trying to selectively decide what
counts as "real drift worth re-baselining to" -- and avoids the failure
mode of a real jump getting stuck as a permanent alarm.
"""
