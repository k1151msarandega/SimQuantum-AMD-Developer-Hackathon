...
pipeline's entire "keep up with a fast stream" premise -- that would be
its own version of the Act I mistake, just relocated into the hot path
instead of the control flow. So this supervisor:
...
path. It's the "slow reasoning layer above a fast reactive layer" pattern
real control systems use, and it's the honest way to give an LLM a real
job in a system whose whole premise is speed. The fast, per-frame FULL/
CHEAP/SKIP decision itself stays exactly as rule-based as it was --
weighing queue depth, staleness, and drift together in one call is what
makes that decision real regardless of who set the thresholds it uses.
