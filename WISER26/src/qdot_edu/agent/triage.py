...
qdot_edu) from qdot-live-twin (Act II), src/qdot_twin/agent/triage.py.
Pure Python, no GPU dependency. See docs/PORTING_NOTES.md.

NOTE for the WISER rebuild: STALE_THRESHOLD_S below was grounded in the
ORIGINAL GPU-batched run's measured worst-case lag (~0.0625s). Once the
CPU-only batch_estimator/serial_estimator are benchmarked on a target
laptop, this should be re-measured and re-grounded the same way -- copy
the method, not just the number. Tracked in docs/PORTING_NOTES.md.
"""
...
# TODO (WISER CPU port): re-measure against the CPU batch_estimator.
