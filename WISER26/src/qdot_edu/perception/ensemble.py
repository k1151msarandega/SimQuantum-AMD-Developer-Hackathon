...
Serves as the real per-frame compute workload for the serial baseline
(twin/serial_estimator.py) and, batched, for the batch estimator
(twin/batch_estimator.py). Weights are untrained/random: irrelevant for
now, since the demo only needs realistic *compute cost and shape*, not
classification accuracy.

Models are cached per-device (see _get_ensemble), all built from the same
seed, so the CPU serial estimator and the batched estimator are comparing
the *same model*, just batched differently.

ensemble_forward supports running a SUBSET of members (n_members) -- this
is what the triage agent's CHEAP tier uses: genuinely less of the same
real computation, not a different heuristic, so its output stays directly
comparable to the FULL tier's.

PORTED UNCHANGED (only the package import path changed, qdot_twin ->
qdot_edu) from qdot-live-twin (Act II), src/qdot_twin/perception/
ensemble.py. Already defaulted to device="cpu" in the original repo --
the GPU-only code lived in twin/batch_estimator.py, not here. See
...
N_CLASSES = 4  # arbitrary placeholder charge-state classes
...
def _suppress_stderr():
    ...
    regardless of where the warning actually originates.
    """
    stderr_fd = 2
...
class _SmallCNN(nn.Module):
    """Conv net: three conv layers, 32/64/128 channels, global pool, linear head."""
...
def _get_ensemble(device: str = "cpu") -> list[_SmallCNN]:
    ...
    device and caching. Same seed regardless of device, so CPU and any
    other-device copies have identical weights -- comparisons stay
    apples to apples.
    """
